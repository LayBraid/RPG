...
for i in range(1, x + 1):
    for j in range(1, y + 1):
        ...
        t.write("\nvoid set_" + str(function_name) + str((i - 1) * x) + str(j - 1) + "(sfTexture *texture, node_rectangle *rectangle)\n{\n "
                                                                  "   sfIntRect rect = {" +
                str(tmp) + "," + str(tmp2) + "," +
        ...
        inc.write("\nadd_next_texture(&texture, set_" + str(function_name) + str((i - 1) * x) + str(j - 1) + ", \"" + str(function_name) +
                  str((i - 1) * x) + str(j - 1) + "\");")
        inc.close()
