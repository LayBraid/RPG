                              ...
                              const float info[4])
{
    ...
    new->next = (*node);
    tmp->next = new;
    data->collisions_dj[(int) new->y][(int) new->x] = new->type;
}
                                ...
                                const float info[4])
{
    ...
    (*node)->id = 0;
    (*node)->next = (*node);
    data->collisions_dj[(int) (*node)->y][(int) (*node)->x] =
            (*node)->type;
}
...
{
    if (data->map_dj == NULL)
        setup_map_rectangle(data, &data->map_dj, info);
    else
        add_map_rectangle(data, &data->map_dj, info);
}
