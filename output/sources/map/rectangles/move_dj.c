...
void dj_move_down(data_t *data)
{
    ...
    if (player.y >= 2250 - 28)
        return;
    if (!can_move(data->collisions_dj[position.y + 1][position.x]))
        return;
    ...
    data->player.position = sfRectangleShape_getPosition(data->
            player.rectangle);
    data->position_dj = sfRectangleShape_getPosition(data->
            player.rectangle);
...
void dj_move_right(data_t *data)
{
    ...
    if (player.x >= 2250 - 16)
        return;
    if (!can_move(data->collisions_dj[position.y][position.x + 1]))
        return;
