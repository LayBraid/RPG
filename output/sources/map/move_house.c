...
void house_move_down(data_t *data)
{
    ...
    if (player.y >= 2250 - 28)
        return;
    if (!can_move(data->collisions_house[position.y + 1][position.x]))
        return;
    ...
    player.rectangle);
    data->position_house = sfRectangleShape_getPosition(data->
    player.rectangle);
...
void house_move_right(data_t *data)
{
    ...
    if (player.x >= 2250 - 16)
        return;
    if (!can_move(data->collisions_house[position.y][position.x + 1]))
        return;
