...
void hyrule_move_up(data_t *data)
{
    ...
    if (!can_move(data->collisions_hyrule[position.y - 1][position.x]))
        return;
    if (data->y_pile_hyrule == 0) {
        data->y_pile_hyrule = 12;
        data->positions_hyrule[position.y - 1][position.x] = PLAYER;
        data->positions_hyrule[position.y][position.x] = 0;
    } else
        data->y_pile_hyrule -= 3;
...
void hyrule_move_left(data_t *data)
{
    ...
    if (player.x <= 0)
        return;
    if (!can_move(data->collisions_hyrule[position.y][position.x - 1]) && data->x_pile_hyrule == 0)
        return;
    sfRectangleShape_move(data->player.rectangle, (sfVector2f) {-3, 0});
    if (data->x_pile_hyrule == 0) {
        data->x_pile_hyrule = 12;
        data->positions_hyrule[position.y][position.x - 1] = PLAYER;
        data->positions_hyrule[position.y][position.x] = 0;
    } else
        data->x_pile_hyrule -= 3;
