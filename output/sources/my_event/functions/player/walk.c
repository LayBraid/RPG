...
void player_walk_down(data_t *data)
{
    ...
    if (data->y_pile == 12) {
        data->y_pile = 0;
        data->positions[position.y + 1][position.x] = PLAYER;
...
void player_walk_right(data_t *data)
{
    ...
    if (data->x_pile == 12) {
        data->x_pile = 0;
        data->positions[position.y][position.x + 1] = PLAYER;
        data->positions[position.y][position.x] = 0;
    } else
        data->x_pile += 3;
...
void player_walk_left(data_t *data)
{
    ...
    if (player.x <= 0)
        return;
    if (!can_move(data->collisions[position.y][position.x - 1]) && data->x_pile == 0)
        return;
    sfRectangleShape_move(data->player.rectangle, (sfVector2f) {-3, 0});
    if (data->x_pile == 0) {
        data->x_pile = 12;
