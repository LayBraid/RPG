...
static int count_requirements(const char *buffer)
{
    int count = 1;
...
static void add_requirement(quest_t *quest, char *name, int calls)
{
...
void extract_requirements(quest_t *quest, char *buffer)
{
    ...
    for (int i = 0; i < count_requirements(buffer); i++) {
        add_requirement(quest, extract_between_limits(buffer,
